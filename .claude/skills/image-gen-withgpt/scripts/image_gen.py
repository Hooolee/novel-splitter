#!/usr/bin/env python3
"""通过第三方 API 生成或编辑图片的 CLI 工具。

支持 generate / edit / generate-batch 三种模式。
默认模型 gpt-image-2，支持结构化 prompt 增强。
"""
...
DEFAULT_ENV_FILE = _SKILL_DIR / "account.env"
...
def _dependency_hint(package: str, *, upgrade: bool = False) -> str:
    ...
    return f"请先安装依赖：pip3 install {flag}{package}"


def _load_account_env(env_file: Optional[Path] = None) -> None:
    """从 account.env 加载凭据到环境变量（已有环境变量优先）。"""
    ...
    # API Key：环境变量 > account.env
    if not os.getenv("OPENAI_API_KEY") and kvs.get("apikey"):
        os.environ["OPENAI_API_KEY"] = kvs["apikey"]

    # Base URL：环境变量 > account.env（自动追加 /v1）
    if not os.getenv("OPENAI_BASE_URL") and kvs.get("apibase"):
        base = kvs["apibase"].rstrip("/")
...
def _ensure_api_key(dry_run: bool) -> None:
    if os.getenv("OPENAI_API_KEY"):
        return
    if dry_run:
        _warn("OPENAI_API_KEY 未设置，仅 dry-run 模式。")
        return
    _die(
        "OPENAI_API_KEY 未设置。请在 account.env 中配置 apikey=... "
        "或导出环境变量 OPENAI_API_KEY。"
    )
